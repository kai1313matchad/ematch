...
class Visit extends CI_Controller {
	...
	function history() {
		...
		if ($submit == 'Tampil') {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$akhir = $this->input->post('selesai');

				// $start = date('Y-m-d', strtotime($awal. ' - 1 days'));
				// $end = date('Y-m-d', strtotime($akhir. ' + 1 days'));
		...
		if ($this->app_model->bollPass()) {
			// $data['list_dept'] =  $this->visit->getDept();
			$data['list_dept'] =  $this->visit->getDeptNew();
    ...
    // 		redirect('Evisit/visit/show','refresh');
    // 	} else {
    // 		redirect('Evisit/visit/dashboard','refresh');
    // 	}	
    // }
